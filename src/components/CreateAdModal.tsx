...
export function CreateAdModal() {
  ...
  useEffect(() => {
    axios("http://10.33.133.143:3333/games").then((response) => {
  ...
  async function handleCreateAd(event: FormEvent) {
    ...
    try {
      await axios.post(`http://10.33.133.143:3333/games/${data.game}/ads`, {
        name: data.name,
        ...
        <form onSubmit={handleCreateAd} className="mt-8 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Input
              id="name"
              name="name"
              placeholder="Como te chamo dentro do game?"
            />
          </div>
          ...
          <footer className="mt-4 flex flex-row justify-end gap-4">
            ...
            <button
              type="submit"
              className="bg-violet-500 px-5 h-12 rounded-md font-semibold flex flex-row items-center gap-3 hover:bg-violet-600"
            >
